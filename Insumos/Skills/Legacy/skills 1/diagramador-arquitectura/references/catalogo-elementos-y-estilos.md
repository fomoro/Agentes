...
|---|---|---|---|---|---|---|---|
| **Zona** | Delimita ecosistemas, redes o dominios de alto nivel | `swimlane; startSize=30; fontStyle=1; align=center; whiteSpace=wrap; html=1; fillColor=<según tipo>; gradientColor=<según tipo>; fontColor=<según tipo>` | **Dinámico (Ajuste orgánico)** | Cabecera superior centrada | `14 px` | Negrita | Sí |
| **Frame** | Agrupa lógica o físicamente componentes y otros frames | `shape=umlFrame; rounded=0; container=1; fontStyle=1; width=150; height=30; whiteSpace=wrap; html=1; fillColor=<según tipo>; strokeColor=#000000` | **Dinámico (Ajuste orgánico)** | Dentro de la pestaña superior | `12 px` | Negrita | Sí |
| **Componente contenedor** | Agrupa uno o más componentes hijos | `rounded=0; whiteSpace=wrap; html=1; fillColor=<según estado>; strokeColor=#000000; verticalAlign=top; container=1; fontStyle=1; spacingTop=6` | Mínimo `200 x 150 px` | Centrada arriba | `11 px` | Negrita | Sí |
| **Componente simple** | No contiene componentes hijos | `rounded=0; whiteSpace=wrap; html=1; fillColor=<según estado>; strokeColor=#000000; verticalAlign=middle` | Mínimo `120 x 50 px` | Centrada adentro | `11 px` | Normal | Sí |
...
| Browser | `shape=image; aspect=fixed; image=img/lib/azure2/general/Browser.svg` | `50 x 50 px` | Debajo del ícono | No |
...
- Cruza el `Frame` y el `Detalle de entorno` confirmados de ambos extremos. Compartir un frame o una zona visual no confirma conectividad.
- Aplica primero una fila que nombre plataformas concretas; usa una fila genérica solo cuando no exista una correspondencia más específica.
- `Conexión Interna` requiere confirmación expresa de la misma suscripción y región en ambos extremos.
- Si falta un dato, ninguna fila coincide o más de una fila conserva la misma precedencia, registra el caso en `Pendientes`; no asignes un tramo.
