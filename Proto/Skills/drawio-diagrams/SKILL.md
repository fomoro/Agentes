...
- Perfil opcional: `default` si no se indica otro, `wolfan` o `colcomercio`. Si se pide uno inexistente, solicita el perfil correcto; no sustituyas silenciosamente la identidad visual.
...
2. Lee el [catálogo](theme/default/catalogo-estilos.md) y la [guía](theme/default/guia-construccion.md) del perfil elegido, sustituyendo `default` por su nombre. Consulta su `inventario-aplicaciones.md` únicamente si necesitas ubicar sistemas; una ubicación no corroborada no demuestra conectividad. No cargues los otros perfiles.
